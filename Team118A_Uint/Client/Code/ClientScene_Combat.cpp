...
CClientScene_Combat::CClientScene_Combat(LPDIRECT3DDEVICE9 pGraphicDev)
	: CScene(pGraphicDev)
{
}

CClientScene_Combat::~CClientScene_Combat()
{
}

CClientScene_Combat * CClientScene_Combat::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CClientScene_Combat*	pInstance = new CClientScene_Combat(pGraphicDev);
...
HRESULT CClientScene_Combat::Ready_Scene()
{
	...
	CUI_Manager::GetInstance()->m_fpComVtory_End_Button = bind(&CClientScene_Combat::End_Battle, this, E_SCENETAG::Scene_TileField);
	CScene::Ready_Scene();
	Ready_Prototype();
	...
	if (nullptr == m_pWorld)
	{
		m_pWorld = CGameManager::GetInstance()->Get_Worlds()->begin()->second.front();
...
_int CClientScene_Combat::Start_Scene(const _float & fTimeDelta)
{  
...
_int CClientScene_Combat::Update_Scene(const _float & fTimeDelta)
{
...
_int CClientScene_Combat::LateUpdate_Scene(const _float & fTimeDelta)
{
...
void CClientScene_Combat::Render_Scene()
{
}

HRESULT CClientScene_Combat::Ready_Prototype()
{
	return S_OK;
}

HRESULT CClientScene_Combat::Ready_LightInfo()
{
...
HRESULT CClientScene_Combat::Binah_OtherSetting()
{
	//Binah Light Setting
	D3DLIGHT9			tLightInfo;
	ZeroMemory(&tLightInfo, sizeof(D3DLIGHT9));
...
HRESULT CClientScene_Combat::Ready_SceneObject()
{
	CGameObject* pGameObject = nullptr;

	// Dynamic Camera
	pGameObject = CDynamicCamera::Create(m_pGraphicDev, g_hWnd,
		T_CAMINFO(_vec3(-2.4f, 2.6f, -2.4f), VEC_ZERO, VEC_UP, D3DXToRadian(60.f), (_float)WINCX / (_float)WINCY, 0.1f, 1000.f), E_OBJTAG::OBJ_DynamicCamera);
	assert(pGameObject);
...
void CClientScene_Combat::Set_FogEvent()
{
...
void CClientScene_Combat::Update_FogEvent(const _float & fTimeDelta)
{
...
void CClientScene_Combat::Active_AutoCam()
{
...
void CClientScene_Combat::Set_Fit_Camera()
{
...
void CClientScene_Combat::Update_Fit_Camera(const _float & fTimeDelta)
{
...
void CClientScene_Combat::Active_ShakeCam(const _float & fShakeTime, const _float & fShakePower)
{
	m_fCamShake_Time = fShakeTime;
	m_fCamShake_Power = fShakePower;
}

void CClientScene_Combat::Update_ShakeCam(const _float & fTimeDelta)
{
...
void CClientScene_Combat::End_Battle(E_SCENETAG eTag)
{
...
void CClientScene_Combat::Spawn_Striker()
{
	map<_int, vector<CCharacter*>>* pMapFormation = CGameManager::GetInstance()->Get_CurrentSquad();
...
void CClientScene_Combat::Spawn_Special()
{
	map<_int, vector<CCharacter*>>* pMapFormation = CGameManager::GetInstance()->Get_CurrentSquad();
...
void CClientScene_Combat::Set_Outtro()
{
	if (m_bEnd)
	{
		return;
...
void CClientScene_Combat::Change_World()
{
	if (!m_pWorld2)
	{
...
void CClientScene_Combat::Play_Game()
{
	if (m_pBattleStartUI &&
...
void CClientScene_Combat::TimeEvent(_float fTimeDelta)
{
	if (m_pWorld->Get_WorldMod() == E_WORLD_MOD::MOD_TOTAL)
	{
		return;
...
void CClientScene_Combat::Update_BattleStartUI(const _float & fTimeDelta)
{
	if (E_WORLD_MOD::MOD_MISSION == m_pWorld->Get_WorldMod())
	{
		if (m_pBattleStartUI &&
