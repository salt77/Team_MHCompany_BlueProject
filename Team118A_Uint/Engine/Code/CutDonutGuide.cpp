...
CCutDonutGuide::CCutDonutGuide(LPDIRECT3DDEVICE9 pGraphicDev, E_OBJTAG eObjTag)
	:CSkillGuideObject(pGraphicDev, eObjTag)
{
}

CCutDonutGuide::CCutDonutGuide(const CCutDonutGuide & rhs)
	:CSkillGuideObject(rhs)
{
}

CCutDonutGuide::~CCutDonutGuide()
{
}

CCutDonutGuide * CCutDonutGuide::Create(LPDIRECT3DDEVICE9 pGraphicDev, E_OBJTAG eObjTag, _float fMinDistance, _float fMaxDistance, _float fAngle, _float fDelay, _int iVertexCount, _bool bUseDelayMesh, wstring wstrTexPath)
{
	CCutDonutGuide* pInstance = new CCutDonutGuide(pGraphicDev, eObjTag);

	if (FAILED(pInstance->Ready_Object(fMinDistance, fMaxDistance, fAngle, fDelay, iVertexCount, bUseDelayMesh, wstrTexPath)))
	{
...
HRESULT CCutDonutGuide::Ready_Object(_float fMinDistance, _float fMaxDistance, _float fAngle, _float fDelay, _int iVertexCount, _bool bUseDelayMesh, wstring wstrTexPath)
{
	...
	m_iGuideVertexCount = m_iVertexCount * 2;
	m_iGuideTriCount = (m_iVertexCount - 1) * 2;
	...
	_float fVertexAngle = - m_fAngle * 0.5f;
	_float fAddAngle = m_fAngle / (_float)(m_iVertexCount - 1);
	...
	{
		D3DXMatrixRotationY(&matRotY, fVertexAngle * Deg2Rad);
		...
		pVertexBuffer[i + m_iVertexCount].vPosition = vVertexLook * m_fMaxDistance;
		pVertexBuffer[i + m_iVertexCount].vPosition.y = m_fAddY;
		pVertexBuffer[i + m_iVertexCount].vTexUV = _vec2(fUVX, 1.f);

		fVertexAngle += fAddAngle;
		fUVX += fAddUVX;
	}
	...
	{
		_int iIndex = i * 2;
		pIndexBuffer[iIndex] = { unsigned short(i), unsigned short(i + m_iVertexCount), unsigned short(i + 1) };
		pIndexBuffer[iIndex + 1] = { unsigned short(i + m_iVertexCount), unsigned short(i + m_iVertexCount + 1), unsigned short(i + 1) };
	}
...
void CCutDonutGuide::Free()
{
	CSkillGuideObject::Free();
}

HRESULT CCutDonutGuide::Ready_Object()
{
	return E_NOTIMPL;
}

_int CCutDonutGuide::Start_Object(const _float & fTimeDelta)
{
	return CSkillGuideObject::Start_Object(fTimeDelta);
	
}

_int CCutDonutGuide::Update_Object(const _float & fTimeDelta)
{
...
_int CCutDonutGuide::LateUpdate_Object(const _float & fTimeDelta)
{
	CSkillGuideObject::LateUpdate_Object(fTimeDelta);
	return RETURN_OK;
}

void CCutDonutGuide::Render_Object(_int iPass)
{
	CSkillGuideObject::Render_Object(iPass);
}

void CCutDonutGuide::Update_DelayMesh()
{
	...
	for (_int i = 0; i < m_iVertexCount; ++i)
	{
		...
		pVertexBuffer[i + m_iVertexCount].vPosition = vVertexLook * fDelayDistance;
		pVertexBuffer[i + m_iVertexCount].vPosition.y = m_fDelayAddY;
		pVertexBuffer[i + m_iVertexCount].vTexUV = _vec2(fUVX, 1.f);
