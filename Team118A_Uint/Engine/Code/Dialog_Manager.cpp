...
void CDialog_Manager::Start_DialogEvent(const wstring& wstrEventName)
{
	...
	//m_pDialogUI->Get_ChildByIndex(0)->Set_ImageAlpha(0.f); 
...
// 이벤트에 필요한 프로토타입 로드(현재는 이미지만 테스트)
void CDialog_Manager::Load_Prototype()
{
	//for (size_t i = 0; i < m_mapDialogInfo[m_wstrEventName].size(); ++i)
	//{
	//	if (L"NULL" != m_mapDialogInfo[m_wstrEventName][i]->wstrCharName)
	//	{
	//		CComponent*		pComponent = CTexture::Create(m_pGraphicDev,
	//			L"../../Reference/Resource/Texture/Dialog/" + m_mapDialogInfo[m_wstrEventName][i]->wstrCharName + L".png",
	//			E_TEXTURE_TYPE::TEX_NORMAL);

	//		if (E_FAIL == CProtoMgr::GetInstance()->
	//			Ready_Prototype(L"Proto_" + m_mapDialogInfo[m_wstrEventName][i]->wstrCharName + L"_Dialog_Texture",
	//				pComponent))
	//		{
...
// 이벤트에 사용된 프로토타입 삭제  
void CDialog_Manager::Clear_Prototype()
{
	for (size_t i = 0; i < m_mapDialogInfo[m_wstrEventName].size(); ++i)
	{
		if (L"NULL" != m_mapDialogInfo[m_wstrEventName][i]->wstrCharName)
		{
			if (E_FAIL == CProtoMgr::GetInstance()->Delete_Prototype(L"Proto_" + m_mapDialogInfo[m_wstrEventName][i]->wstrCharName + L"_Dialog_Texture"))
			{
				continue;
			}
		}
...
void CDialog_Manager::Emplace_MapDialogInfo(const wstring & wstrEventName, T_DialogInfo * tDialogInfo)
{
	if (m_mapDialogInfo[wstrEventName].empty())
	{
		m_mapDialogInfo[wstrEventName].reserve(200);
	}
...
void CDialog_Manager::Render_DialogFont()
{
	...
	if (L"" != m_wstrEventName)
	{
		...
		{
			if (L"???" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName ||
				L"아로나" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName ||
				L"..." == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName)
			{
...
_bool CDialog_Manager::Dialog_EndEpisode()
{
	if (m_pDialogEnd)
	{
		if (!m_bDialogEndBrake)
		{
			...
			m_pDialogEnd->Set_ImageAlpha(0.f);
			//m_pDialogEnd->Set_Ui_Text(m_mapDialogInfo[m_wstrEventName][0]->wstrEpisodeNext);
...
void CDialog_Manager::Update_Emoticon()
{
	...
	case CDialog_Manager::Emoticon_Alarm:
		//m_pEmoticonUI->Get_ChildByIndex(Emoticon_Alarm)->Set_Active(TRUE);
		//m_fEmoticonBlinkCountTime += CTimeScale_Manager::GetInstance()->Get_TimeDelta();

		//if (m_fEmoticonBlinkDelay <= m_fEmoticonBlinkCountTime)
		//{
		//	m_fEmoticonBlinkCountTime = 0.f;

		//	m_pEmoticonUI->Get_ChildByIndex(Emoticon_Alarm)->Set_Active(m_pEmoticonUI->Get_ChildByIndex(Emoticon_Alarm)->Get_Active() ? FALSE : TRUE);
		//}
		break;
...
void CDialog_Manager::Middle_DialogFont()
{
	if (L"" != m_wstrEventName)
	{
		if (L"NULL" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName &&
			L"Middle" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrDialogFontPos)
		{
			CFontMgr::GetInstance()->Render_FontEveryFrame(T_RENDERFONT(FontTag_경기천년제목_Medium, m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrTextDialog, Dialog_MiddleUp, D3DXCOLOR(1.f, 1.f, 1.f, m_fTimeCount), 13, 22, FW_NORMAL, FALSE));

			if (m_fMiddleFontTurnOver <= m_fTimeCount)
			{
				CFontMgr::GetInstance()->Render_FontEveryFrame(T_RENDERFONT(FontTag_경기천년제목_Medium, m_mapDialogInfo[m_wstrEventName][m_iDialogIndex + 1]->wstrTextDialog, Dialog_Middle, D3DXCOLOR(1.f, 1.f, 1.f, m_fTimeCount - m_fMiddleFontTurnOver), 13, 22, FW_NORMAL, FALSE));
			}
			if (m_fMiddleFontTurnOver * 2.f <= m_fTimeCount)
			{
				CFontMgr::GetInstance()->Render_FontEveryFrame(T_RENDERFONT(FontTag_경기천년제목_Medium, m_mapDialogInfo[m_wstrEventName][m_iDialogIndex + 2]->wstrTextDialog, Dialog_MiddleDown, D3DXCOLOR(1.f, 1.f, 1.f, m_fTimeCount - m_fMiddleFontTurnOver * 2.f), 13, 22, FW_NORMAL, FALSE));
			}
			if (m_fMiddleFontTurnOver * 3.f <= m_fTimeCount)
			{
				m_iDialogIndex += 3;
...
_bool CDialog_Manager::Branch_Event()
{
	if (L"Branch" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName)
	{
		...
		if (L"NULL" != m_mapDialogInfo[m_wstrEventName][m_iDialogPreIndex]->wstrCharName)
		{
			if (L"???" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName ||
				L"아로나" == m_mapDialogInfo[m_wstrEventName][m_iDialogIndex]->wstrCharName ||
				L"???" == m_mapDialogInfo[m_wstrEventName][m_iDialogPreIndex]->wstrCharName ||
				L"아로나" == m_mapDialogInfo[m_wstrEventName][m_iDialogPreIndex]->wstrCharName)
			{
