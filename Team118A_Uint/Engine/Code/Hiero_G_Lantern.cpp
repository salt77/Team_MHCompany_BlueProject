...
CHiero_G_Lantern::CHiero_G_Lantern(LPDIRECT3DDEVICE9 pGraphicDev, E_OBJTAG eObjTag)
	: CGameObject(pGraphicDev, eObjTag)
{
}

CHiero_G_Lantern::~CHiero_G_Lantern()
{
}

CHiero_G_Lantern * CHiero_G_Lantern::Create(LPDIRECT3DDEVICE9 pGraphicDev, E_OBJTAG eObjTag)
{
	CHiero_G_Lantern* pInstance = new CHiero_G_Lantern(pGraphicDev, eObjTag);
...
void CHiero_G_Lantern::Free()
{
	CGameObject::Free();
}

HRESULT CHiero_G_Lantern::Ready_Object()
{
...
_int CHiero_G_Lantern::Update_Object(const _float & fTimeDelta)
{
	...
	if (0.f < m_fActiveTime)
	{
		...
		{
			iter->Set_Invn((m_fRadius * 0.5f >= CUtility::Distance_vec3(Get_Position(), iter->Get_Position())));
		}
	}
	else
	{
...
_int CHiero_G_Lantern::LateUpdate_Object(const _float & fTimeDelta)
{
...
void CHiero_G_Lantern::Render_Object(_int iPass)
{
	Set_Constantable();

	m_pShaderCom->BeginShader();
	m_pShaderCom->BeginPass(ShaderPass(E_ShaderPass::Base, iPass));
	m_pSMeshCom->Set_Glow_Option(0, 0.5f, _vec3(0.f, 0.f, 0.f), 1.f, 0.f, 1.f, 0.f, 0.5f);
	if (m_pSMeshCom)
	{
...
void CHiero_G_Lantern::Set_ActiveLantern(const _bool & bState)
{
	...
	CSound_Manager::GetInstance()->Add_Sound_NotOverlap(L"SFX_Hieronymus_HolyRelic_Green.wav", 0.1f);
	for (_int i = 0; i < 3; ++i)
	{
		CEffectObject* pEffect = CEffectObject::Create(m_pGraphicDev, E_OBJTAG::OBJ_Effect, Get_Position(), VEC_ZERO, L"Hieronymus_Lantern_G_Wave.effect");
...
void CHiero_G_Lantern::Add_Component()
{
...
void CHiero_G_Lantern::Set_Constantable()
{
