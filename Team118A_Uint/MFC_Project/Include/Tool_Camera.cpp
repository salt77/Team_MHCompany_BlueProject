// C:\Users\bicus\Documents\GitHub\Team_MHCompany\Team118A_Uint\MFC_Project\Include\Tool_Camera.cpp : 구현 파일입니다.
//
...
{
	if (m_eCamActionType != m_ePreCamActionType)
	{
...
void CTool_Camera::Create_ControlSphere()
{
	...
	if (m_pScene->Get_Layer(E_LAYERTAG::GameLogic)->Get_GameObject(E_OBJTAG::OBJ_DynamicCamera))
	{
		if (Deploy_Normal == m_eDeployType)
		{
			...
			m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].emplace_back(static_cast<CMFC_CamControl_Eye*>(pGameObject));
			m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].resize(m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size());
			m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].emplace_back(1.f);
			...
			m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].emplace_back(static_cast<CMFC_CamControl_At*>(pGameObject));

			// List에 String 추가 
			WCHAR	wszBuff[16];
			swprintf_s(wszBuff, L"%zu", m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size() - 1);
			m_ListControlPoint_Eye.AddString(wszBuff);
			m_ListControlPoint_At.AddString(wszBuff);

			m_mapDeployType[m_cstrCamCurText.operator LPCWSTR()] = Deploy_Normal;
		}
		else if (Deploy_OneAt == m_eDeployType)
		{
			// Control_Eye 
			CGameObject*	pGameObject =
				CMFC_CamControl_Eye::Create(
			...
			// Control_At 
			if (m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].empty())
			{
				// Control_At 
				pGameObject =
					CMFC_CamControl_At::Create(
			...
			m_mapDeployType[m_cstrCamCurText.operator LPCWSTR()] = Deploy_OneAt;
		}
	}
}
...
void CTool_Camera::Refresh_ControlListString()
{
	...
	for (size_t i = 0; i < m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size(); ++i)
	{
		swprintf_s(wszBuff, L"%zu", i);

		m_ListControlPoint_Eye.AddString(wszBuff);

		if (1 < m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size())
		{
			m_ListControlPoint_At.AddString(wszBuff);
		}
	}
...
{
	//if ((_int)m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size() - 2 > m_iCurLineIndex)
	//{
	//	m_fLineLength = D3DXVec3Length(&(m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iCurLineIndex]->Get_Info(E_INFO::INFO_POS) -
	//		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iCurLineIndex + 1]->Get_Info(E_INFO::INFO_POS)));

	//	m_fLineLength *= 5.f;
	//}

	if (m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].empty())
	{
		return;
	}
	...
	if (1.f <= m_fProgressCam)
	{
		...
		if ((_int)m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size() - 3 > m_iCurLineIndex)
		{
			++m_iCurLineIndex;
		}
		else
	...
	m_fProgressCam = m_fTime / m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()][m_iCurLineIndex]; 
}

// 곡선보간(Eye)
void CTool_Camera::Eye_Line()
{
	vector<CMFC_CamControl_Eye*>	vecTemp = m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()];
...
// 곡선보간(At)
void CTool_Camera::At_Line()
{
	vector<CMFC_CamControl_At*>		vecTemp = m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()];
	...
	if (1 == vecTemp.size())
	{
		if (ActionType::Action_Subject == m_eCamActionType)
		{
			m_pMainCam->Set_CamAt(+vecTemp[0]->Get_Info(E_INFO::INFO_POS));
		}
	}
}
...
void CTool_Camera::Click_Action()
{
	...
	case CTool_Camera::Action_Subject:
		if (!m_mapActionEye.empty() &&
			!m_mapActionAt.empty() &&
			!m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].empty())
		{
			m_eCamActionType = (ActionType)m_ComboCamType.GetCurSel();
			...
			assert(m_pMainCam);
			m_pMainCam->Set_CamEye(+m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][0]->Get_Info(E_INFO::INFO_POS));
	...
	case CTool_Camera::Action_Observe:
			...
			!m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].empty())
		{
			...
			m_pMainCam = static_cast<CDynamicCamera*>(Management Get_GameObject(E_LAYERTAG::GameLogic, E_OBJTAG::OBJ_DynamicCamera, 0));
			assert(m_pMainCam);
			m_pMainCam->Set_CamAt(+m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][0]->Get_Info(E_INFO::INFO_POS));
...
void CTool_Camera::OnBnClickedCamAdd()
{
	...
	{
		if (cstrName.operator LPCWSTR() == iter->first)
		{
			bDupl = true;
		}
	}
	...
	if (L"" != m_cstrCamNameInsert &&
		!bDupl)
	{
		m_mapActionEye.emplace(m_cstrCamNameInsert.operator LPCWSTR(), vecEye);
		m_mapActionAt.emplace(m_cstrCamNameInsert.operator LPCWSTR(), vecAt);
...
void CTool_Camera::OnBnClickedCamDelete()
{
	if (-1 != m_iCamCurSel)
	{
			...
			ActionType::Action_Observe != m_eCamActionType)
		{
			for (size_t i = 0; i < m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size(); ++i)
			{
				m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][i]->Set_Dead(TRUE);
			}
			for (size_t i = 0; i < m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size(); ++i)
			{
				m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][i]->Set_Dead(TRUE);
			}

			m_mapActionEye.erase(m_cstrCamCurText.operator LPCWSTR());
			m_mapActionAt.erase(m_cstrCamCurText.operator LPCWSTR());
			m_mapSectionSpeed.erase(m_cstrCamCurText.operator LPCWSTR());
			...
			//if (!m_mapActionEye.empty())
			//{
			//	m_ListCam.GetText(0, m_cstrCamCurText);
...
void CTool_Camera::OnBnClickedControlPointDelete()
{
		...
		(-1 != m_iControlCurSel_Eye || -1 != m_iControlCurSel_At))
	{
		...
		{
			if (1 < m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size() &&
				3 < m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size())
			{
				if (-1 != m_iControlCurSel_Eye)
				{
					m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Set_Dead(TRUE);
					m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Set_Dead(TRUE);

					m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_Eye);
					m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_Eye);
					m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_Eye);

					m_ListControlPoint_Eye.DeleteString(m_iControlCurSel_Eye);
					m_ListControlPoint_At.DeleteString(m_iControlCurSel_Eye);
				}
				else if (-1 != m_iControlCurSel_At)
				{
					m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Set_Dead(TRUE);
					m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Set_Dead(TRUE);

					m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_At);
					m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_At);
					m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_At);

					m_ListControlPoint_Eye.DeleteString(m_iControlCurSel_At);
					m_ListControlPoint_At.DeleteString(m_iControlCurSel_At);
				}
			}
			else if (1 == m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size())
			{
				if (Control_EyePoint == m_eControlPointType)
				{
					if (3 < m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size())
					{
						m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Set_Dead(TRUE);
						m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_Eye);
						m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].erase(m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].begin() + m_iControlCurSel_Eye);
...
// 버튼만 존재하고 기능 x. (씬이 로드되면 어차피 맵도 불러와지기 때문에..)
void CTool_Camera::OnBnClickedCamSceneLoad()
{
}
...
void CTool_Camera::OnBnClickedControlPointPosApply()
{
	...
	case CTool_Camera::Control_EyePoint:
		...
		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][iCurSel]->Set_Pos(vPos);
		break;
	...
	case CTool_Camera::Control_AtPoint:
		...
		m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][iCurSel]->Set_Pos(vPos);
		break;
...
void CTool_Camera::OnSelchangeCamList()
{
		...
		ActionType::Action_Subject != m_eCamActionType)
	{
		if (-1 != m_iCamCurSel)
		{
			...
			{
				if (m_cstrCamCurText.operator LPCWSTR() == iter_Eye->first)
				{
					Refresh_ControlListString();
					...
					if (0 < (iter_Eye->second).size())
					{
						for (size_t i = 0; i < iter_Eye->second.size(); ++i)
						{
							...
							if (i <= m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size() - 1)
							{
								m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][i]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_NORMAL);
							}
						}
					}
				}
			...
			m_eDeployType = m_mapDeployType[m_cstrCamCurText.operator LPCWSTR()];
			m_ePreDeployType = m_eDeployType;
...
void CTool_Camera::OnSelchangeControlPointList_Eye()
{
	...
	size_t	vecSize = m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size();

	if (-1 != m_iCamCurSel &&
		-1 != m_iControlCurSel_Eye)
	{
		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_SELECTED);

		for (size_t i = 0; i < vecSize; ++i)
		{
			if (i != m_iControlCurSel_Eye)
			{
				m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][i]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_NORMAL);
			}
		}
	}
	...
	if (3 < m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size())
	{
		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][0]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_INTERPOL);
		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][vecSize - 1]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_INTERPOL);
		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][vecSize - 2]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_INTERPOL);
	}

	if (-1 != m_iControlCurSel_Eye)
	{
		// Pos Edit에 업데이트 
		_vec3	vPos = m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Get_Info(E_INFO::INFO_POS);
		...
		if (!m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].empty())
		{
			swprintf_s(wszBuff, L"%.2f", m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]);
		}
		else
		{
...
void CTool_Camera::OnSelchangeControlPointList_At()
{
	...
	{
		m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_SELECTED);
		...
		for (size_t i = 0; i < m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size(); ++i)
		{
			if (i != m_iControlCurSel_At)
			{
				...
				{
					m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][0]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_NORMAL);
				}
			}
		}
	}
	...
	if (1 < m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size())
	{
		m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][0]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_INTERPOL);
		m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][vecSize - 1]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_INTERPOL);
		m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][vecSize - 2]->Set_RenderType(E_WIRE_RENDER::WIRE_RENDER_INTERPOL);
	}

	if (-1 != m_iControlCurSel_At)
	{
		_vec3	vPos = m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Get_Info(E_INFO::INFO_POS);
...
void CTool_Camera::OnCbnSelchangeDeployType()
{
	...
	case CTool_Camera::Deploy_Normal:
		if (m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].empty() ||
			1 != m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].size())
		{
			m_ePreDeployType = m_eDeployType;
		}
		else
...
void CTool_Camera::OnBnClickedSpeedApply()
{
	if (-1 != m_iCamCurSel &&
		-1 != m_iControlCurSel_Eye)
	{
		m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].reserve(m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].size());

		UpdateData(TRUE);

		m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye] = (_float)_ttof(m_cstrControlSpeed);
...
void CTool_Camera::OnBnClickedCamToolSAVE()
{
	...
	if (IDOK == Dlg.DoModal())
	{
		...
		for (; iter_At != m_mapActionAt.end(); ++iter_At)
		{
			...
			{
				//(*iter_ControlEye)->Save(hFile, dwbyte);
...
void CTool_Camera::OnEnChangeCurPointSpeed()
{
	...
	if (L"-" == cstrValue || m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].empty())
	{
		return;
	}
		...
		-1 != m_iControlCurSel_Eye)
	{
		...
		m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye] = fTime;
	}
...
void CTool_Camera::OnEnChangeAllChange()
{
	...
	if (-1 != m_iCamCurSel)
	{
		for (_uint i = 0; i < m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()].size(); ++i)
		{
			_float	fTime = (_float)_ttof(m_cstrAllSpeed);
			fTime = CUtility::Clamp_float(fTime, 0.01f, fTime);
			m_mapSectionSpeed[m_cstrCamCurText.operator LPCWSTR()][i] = fTime;
		}
	}
...
void CTool_Camera::OnEditChangePosX()
{
	...
	case CTool_Camera::Control_EyePoint:
		...
		vPos = m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Get_Position();
		vPos.x = m_fPosX;
		m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Set_Pos(vPos);
		break;
	...
	case CTool_Camera::Control_AtPoint:
		...
		vPos = m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Get_Position();
		vPos.x = m_fPosX;
		m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Set_Pos(vPos);
		break;
	case CTool_Camera::Control_End:
		return;
...
void CTool_Camera::OnBnClickedButtonPickObjPosApply()
{
	...
	if (ControlPointType::Control_EyePoint == m_eControlPointType)
	{
		if (-1 != m_iControlCurSel_Eye && 
			!m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()].empty())
		{
			m_mapActionEye[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_Eye]->Set_Pos(Get_PopupTool_PickObject()->Get_Position()); 
		}
	}
	else if (ControlPointType::Control_AtPoint == m_eControlPointType)
	{
		if (-1 != m_iControlCurSel_At &&
			!m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()].empty())
		{
			_vec3	vTargetPos = Get_PopupTool_PickObject()->Get_Position(); 
			vTargetPos.y += 0.75f; 

			m_mapActionAt[m_cstrCamCurText.operator LPCWSTR()][m_iControlCurSel_At]->Set_Pos(vTargetPos);
		}
	} 
}
